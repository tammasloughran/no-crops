...
CROP_INDEX = 8
DPI = 200
EXAMPLE_FLIE = f'{ARCHIVE_DIR}/GWL-NoCrops-B2030/pr_GWL-NoCrops-B2030_0500-0700.nc'
EXPERIMENTS = {
...
def yearly_mean_from_monthly(data:np.ndarray)->np.ndarray:
    ...
    """
    NMONTHS = 3 # Change for annual mean
    if np.mod(data.shape[0], NMONTHS)!=0:
        raise ValueError("Not enough months in 0th dimension.")
    toshape = list(data.shape)
    toshape.pop(0)
    toshape.insert(0, NMONTHS)
    toshape.insert(0, int(data.shape[0]/NMONTHS))
    #fraction_of_year = np.array([31,28,31,30,31,30,31,31,30,31,30,31])/365.0
    fraction_of_year = np.array([30,31,31,])/92.0 # For winter mean
...
def main()->None:
    ...
    for gwl_exp,nocrop_exp in EXPERIMENTS.items():
        ...
        # Find the difference for pr.
        # 2nd ensemble members start at year 510
        if '-02' in nocrop_exp:
            pr_diff = data[nocrop_exp]['pr'] - data[gwl_exp]['pr'][12*10:]
            pr_for_diff = data_forest_only[nocrop_exp]['pr'] - \
                    data_forest_only[gwl_exp]['pr'][12*10:]
        else:
            pr_diff = data[nocrop_exp]['pr'] - data[gwl_exp]['pr']
            pr_for_diff = data_forest_only[nocrop_exp]['pr'] - data_forest_only[gwl_exp]['pr']

        ## Plot the difference time series of pr.
        plt.figure(1)
        nmonths = pr_diff.squeeze().shape[0]
        ...
        nmonths = pr_diff.squeeze().shape[0]
        years = np.linspace(500, 500+nmonths*(1/12), nmonths)
        yearly_pr_for_diff = yearly_mean_from_monthly(pr_for_diff.squeeze())
        #plt.plot(yearly_mean_from_monthly(years), yearly_pr_for_diff,
        #        color=COLORS[nocrop_exp],
        #        alpha=0.3,
        #        )
    ...
    """
    NEXP = 9
    NTIMES = len(yearly_mean_from_monthly(data['PI-GWL-t6']['pr']))
    references = np.ones((NEXP,NTIMES))
    experiments = references.copy()
    i = 0
    for ref,exp in EXPERIMENTS.items():
        if 'duplicate' in ref: ref = ref[:-10]
        length = len(yearly_mean_from_monthly(data[exp]['pr'].squeeze()))
        references[i,:] = yearly_mean_from_monthly(data[ref]['pr'].squeeze())
        experiments[i,:length] = yearly_mean_from_monthly(data[exp]['pr'].squeeze())
        i += 1
    ...
    plt.savefig(f'plots/pr_GWL_australia.png', dpi=DPI)

    plt.figure(2)
    decorate_plot()
    plt.title('Australia forestation area mean')
    plt.savefig(f'plots/pr_GWL_australia_forestation_only.png')
    plt.show()
